...
class AntiNuke(commands.Cog):
    ...
    async def punish(self, guild, user, reason):
        ...
        if count == 1:
            await self.send_log(guild, "antinuke", f"⚠️ Warned {member}")
        elif count == 2:
            await member.kick()
            await self.send_log(guild, "antinuke", f"👢 Kicked {member}")
        else:
            await member.ban()
            await self.send_log(guild, "antinuke", f"🔨 Banned {member}")
    ...
    async def check(self, guild, action, audit):
        ...
        async for entry in guild.audit_logs(limit=1, action=audit):
            ...
            if count >= LIMITS[action]:
                await self.send_log(guild, "antinuke", f"💣 {user} exceeded {action} limit ({count})")
    ...
    async def on_guild_role_delete(self, role):
        ...
        await self.send_log(role.guild, "antinuke", f"⚠️ Role deleted: {role.name}")
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        await self.send_log(channel.guild, "antinuke", f"⚠️ Channel deleted: {channel.name}")
    ...
    async def on_guild_role_update(self, before, after):
        if before.permissions != after.permissions and self.dangerous(after):
            ...
            await self.send_log(after.guild, "antinuke", f"⚠️ Dangerous permissions given: {after.name}")
